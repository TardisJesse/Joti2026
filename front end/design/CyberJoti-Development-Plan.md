...
| 2026-09-19 | Fixed and verified | Railway frontend-to-backend authentication proxy fixed. The frontend initially targeted the backend's development port 8000, while Railway assigned the running backend port 8080; the private `BACKEND_URL` now targets port 8080 and requests no longer fail with 502. |
| 2026-09-19 | Ready for publish | Added environment-driven MVP admin bootstrap. Railway variables hold the test account rather than the repository; after the source commit deploys, `admin` is an active administrator. The API requires an 8-character minimum password, so the test password is `admin123`. |
| 2026-09-19 | Planned | Next MVP task definition written: passwordless player entry by game code and unique team name, phone GPS as the location source, privacy-separated player/admin maps, and explicit admin game-round switching. See `CyberJoti-MVP-Task-Definition.md`. |
| 2026-09-19 | Done and verified | Implemented passwordless team entry: a game code creates or joins a running round, a unique team name creates a persistent team session, and player API data is scoped to that team and game. Added game-code/team-name migration, explicit game-scoped admin object APIs, admin round APIs, game-scoped score/location queries, and WebSocket game channels. Local migration, join-flow smoke test, and Vue production build pass. |
| 2026-09-19 | Updated and verified | Game-round ownership rules refined: unknown player game codes now return a clear error instead of creating a round; only admins can create rounds, which begin in `READY`. Puzzle, Capture the Flag, and NFC scoring now require the admin to set that specific round to `RUNNING`. Unknown-code API smoke test returns 404 as intended. |
| 2026-09-19 | Done and verified | Admin UI now has separate **Spel rondes** and **Objecten** subtabs. Admins can create a code, choose a round, start/pause it, and place objects only in that selected round. Vue production build passes. |
| 2026-09-19 | Done and verified | The radar map now starts at Allart van Heemstede and moves to the phone GPS location only once when that location first becomes available. Subsequent data refreshes and returning to the map preserve the player's manual pan and zoom. Object-create feedback now uses a five-second, auto-clearing status message instead of a blocking pop-up. Vue production build passes. |
...
| 2026-09-19 | Implemented; build and behavior verified | Mobile geo UX plan implemented: mobile viewport, available-height radar, working filters, selectable points and collapsible details/list, distance and game actions, explicit recenter control, truthful GPS state and immediate local location display, larger touch targets and safe-area spacing. See `Mobile-Geo-UX-Plan.md` for sources and checklist. Production build and MapLibre-double behavior tests pass; actual phone/browser visual checks remain open because the browser integration failed to initialize. |
